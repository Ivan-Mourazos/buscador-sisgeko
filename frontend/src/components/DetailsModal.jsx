    ...
    return (
        ...
        >
            ...
            >
                ...
                <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-start bg-gray-50/50 backdrop-blur-xl">
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-3">
                            <span className={`px-2.5 py-1 text-[11px] font-black uppercase rounded-lg tracking-widest shadow-sm ${
                                item._type === 'articulo' ? 'bg-indigo-600 text-white' :
                            ...
                            <div className="flex flex-wrap gap-x-8 gap-y-2 mt-2">
                                {item.denominacion_proveedor && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-base font-extrabold text-indigo-700 bg-indigo-50 px-3 py-1 rounded-md border border-indigo-100/50">{item.denominacion_proveedor}</span>
                                    </div>
                                )}
                                {item.codigo && (
                                    <div className="flex items-center gap-2">
                ...
                <div className="p-8 overflow-y-auto custom-scrollbar flex-grow bg-white">
                    <div className="max-w-4xl mx-auto space-y-10">
                                ...
                                <div className="flex flex-wrap gap-4">
                                    {details.imagenes.map((img, idx) => (
                                        <div 
                                            ...
                                            className="relative group cursor-zoom-in overflow-hidden rounded-2xl border-2 border-gray-100 hover:border-indigo-400 transition-all shadow-sm w-[160px] h-[100px] bg-gray-50 flex-shrink-0"
                                        >
                                            <img 
                                                src={getImgUrl(img)} 
                                                alt={`Thumb ${idx}`} 
                        ...
                        <div className="space-y-12">
                            {item._type === 'insight' && (
                                <div className="p-8 bg-blue-50/20 border border-blue-100 rounded-[2rem] relative overflow-hidden ring-1 ring-blue-50">
                                    <div className="absolute top-0 right-0 p-6 text-blue-100/30">
                                    ...
                                    <p className="text-gray-800 text-2xl font-semibold leading-relaxed relative z-10 italic">"{item.insight}"</p>
                                </div>
                            )}

                            {item._type === 'definicion' && (
                                <div className="p-10 bg-purple-50/20 rounded-[2.5rem] border border-purple-100 ring-1 ring-purple-50 italic font-serif">
                                    <p className="text-gray-800 text-2xl leading-relaxed">{details?.textoCompleto || item.definicion}</p>
                                </div>
                            )}

                            {loading ? (
                                <div className="flex flex-col items-center py-20">
                                    <div className="w-12 h-12 border-4 border-gray-100 border-t-emerald-500 rounded-full animate-spin mb-6"></div>
                                    <span className="text-xs font-black uppercase text-gray-400 tracking-widest">Consultando Sisgeko...</span>
                                </div>
                            ) : (
                                        ...
                                        <section>
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                                {details.caracteristicas.map((car, idx) => (
                                                    <div key={idx} className="bg-gray-50/50 hover:bg-white rounded-[1.5rem] p-6 border border-gray-100 shadow-sm hover:shadow-xl transition-all hover:border-indigo-100 group">
                                                        <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.1em] mb-2 group-hover:text-indigo-600 transition-colors">{car.caracteristica.trim()}</h4>
                                    ...
                                    {item._type === 'insight' && (
                                        <div className="flex flex-col gap-10">
                                            ...
                                            <section className="animate-in fade-in slide-in-from-top-4 duration-500">
                                                <h3 className="text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-3">
                                                    Detalles de la fuente y recursos
                                                    <span className="h-[1px] flex-grow bg-gray-100"></span>
                                                </h3>
                                                
                                                <div className="bg-gray-50/80 p-8 rounded-[2rem] border border-gray-100 ring-1 ring-gray-200/50 shadow-sm">
                                                    <div className="flex flex-col lg:flex-row gap-8 items-start">
                                                        {/* PARTE IZQUIERDA: INFORMACIÓN DE REFERENCIA (APILADO VERTICAL) */}
                                                        <div className="flex flex-col gap-6 min-w-[300px] flex-shrink-0">
                                                            <div className="flex flex-col gap-2">
                                                            ...
                                                            <div className="flex flex-col gap-2">
                                                                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Tipología de fuente</span>
                                                                <span className="inline-flex self-start bg-gray-900 text-white px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-tighter shadow-md">
                                                                    {item.tipo_origen_nombre || 'General'}
                                                                </span>
                                                            </div>
                                                        </div>
                                                            ...
                                                            <div className="flex-grow flex flex-col gap-3">
                                                                ...
                                                                <div className="flex flex-wrap gap-4">
                                                                    <div 
                                                                        ...
                                                                        className="relative group cursor-zoom-in overflow-hidden rounded-2xl border-2 border-white hover:border-blue-400 transition-all shadow-lg hover:shadow-xl w-[200px] h-[130px] bg-white ring-1 ring-black/5"
                                                                    >
                                                                        <img 
                                                                        ...
                                                                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-gray-900/5 transition-opacity">
                                                                            <div className="bg-white/95 p-2 rounded-full shadow-2xl scale-75 group-hover:scale-100 transition-transform duration-300">
                                                                                <svg className="w-5 h-5 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M21 21l-6-6M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                            ...
                                            <section className="animate-in fade-in slide-in-from-top-4 duration-700">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    {details?.intenciones?.map((int, i) => (
                                                        <div key={i} className="flex gap-4 text-base text-gray-700 bg-white p-5 rounded-2xl border border-gray-100 font-bold group shadow-sm hover:border-emerald-100 hover:shadow-md transition-all">
                                                            <div className="w-6 h-6 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center shrink-0 text-xs font-black">?</div>
                                                            {int}
                                                        </div>
